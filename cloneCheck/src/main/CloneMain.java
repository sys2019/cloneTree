		...
		for (int i=0; i < args.length; i++) {
			if (args[i].charAt(0) == '-') {
				if (args[i].equals("-i")) {
					...
					i++;
					FileRW.threshold = Integer.parseInt(args[i]);
				}
				else if(args[i].equals("-v")) {
					i++;
					FileRW.min_value = Double.parseDouble(args[i]);
				}
				else if(args[i].equals("-std")) {
					std_flag = true;
				}
				else if(args[i].equals("-boott")) {
					i++;
					boot = Integer.parseInt(args[i]);
				}
				else if(args[i].equals("-bootc")) {
					i++;
					bootc = Double.parseDouble(args[i]);
				}
				else if(args[i].equals("-boots")) {
					i++;
					boots = Double.parseDouble(args[i]);
				}
				else {
					System.out.println("Unkown param: " + args[i]);
					return;
				}
			}
		}
		run(in_file, out_prefix, std_flag, boot, bootc, boots, clone_file);
	}
	
	public static void run(String in_file, String out_prefix, boolean std_flag, int bootstrap, double cluster_up, double remove_up, String clone_file) {
		if (in_file == null || out_prefix == null) {
			System.out.println("ERROR: in/out file error");
			return;
		}
		String name = "Default";
		String[] path = in_file.split("[/_.]");
		for (int i=0; i < path.length; i++) {
			if (path[i].length() > 0 && path[i].charAt(0) == 'P') {
				name = path[i];
			}
		}
